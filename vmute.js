...
if(!sure) return message.channel.send(new MessageEmbed().setTitle('🔴 BU BİR UYARI MESAJIDIR.').setDescription(`${message.author}, Bir zaman belirtmelisin.`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000}));
