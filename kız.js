...
const tag = '▪'   
const kadınrol = message.guild.roles.cache.find(r => r.id === '825831745710063726') //kadın rol id
const kadınrol2 = message.guild.roles.cache.find(r => r.id === '826729881576013846')//kadın rol2 id
const kayıtsız = message.guild.roles.cache.find(r => r.id === '825831748867850312')//kayıtsız rol id
const genelchat = message.guild.channels.cache.find(c => c.id === '825831890770460742')//genel chat id
const savelog = message.guild.channels.cache.find(c => c.id === '826011720471281664')//savelog id
...
Sunucumuzun **◭** tagını alarak ailemizin parçası olabilirsin. `)
  
savelog.send(new MessageEmbed()
.setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
.setTitle(' ✔️ KAYIT İŞLEMİ BAŞARILI BİR ŞEKİLDE TAMAMLANDI !')
