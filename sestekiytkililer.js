...
if(!["825831688151498752"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR"))) 
...
//-------------------------------------------------------------------------------\\
  ...
  let cortex = "**Sesli Kanalda Olan Yetkililer:**\n";
  let cortex2 = "**Sesli Kanalda Olmayan Yetkililer:**\n";
  message.guild.roles.cache.get("yetkili rolü").members.map(r => {
    cortex += r.voice.channel ? "•  <@" + r.user.id + ">\n" : "";
    cortex2 += !r.voice.channel ? "•  <@" + r.user.id + ">\n" : "";
  });
